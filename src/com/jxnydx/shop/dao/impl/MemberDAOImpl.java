...
public class MemberDAOImpl extends AbstractDAOImpl implements IMemberDAO {
	...
	@Override
	public List<Member> findAllByStatus(Integer status, Integer currentPage, Integer lineSize, String column,
			String keyWord) throws Exception {
		List<Member> all = new ArrayList<Member>();
    	String sql = "SELECT mid,password,name,phone,address,code,status,regdate,photo FROM member WHERE " + column + " LIKE '%" + keyWord + "%'"
				+ " AND status=" + String.valueOf(status) + " LIMIT " + (currentPage - 1) * lineSize + "," + lineSize;
    	super.pstmt = super.conn.prepareStatement(sql);
    	/*super.pstmt.setString(1, "%" + keyWord + "%");
